...
APP_KEY = 'A167C61A-10CF-11E9-BA61-CBB570C49BBF'
########################################################
...
COORDINATES_FILE = 'coords_cal.csv'
                ...
                if not bad_start < dt_obj < bad_end:
                    ...
                    if file_ids:
                        ...
                        order_params = {'email': email_address,
                                        'doMosaic': mosaic,
                                        'fileIds': ','.join(file_ids),
                                        'geoSubsetNorth': search_params['north'],
                                        'geoSubsetSouth': search_params['south'],
                                        'geoSubsetEast' : search_params['east'],
                                        'geoSubsetWest' : search_params['west'],
                                        }
...
def combining_fn(email_address=EMAIL, token=APP_KEY, dates=DATE_FILE,
                 ...
                 output_dir=DESIRED_DIR, mosaic=True):
    '''
...
if __name__ == '__main__':
    ...
    order_ids = combining_fn(ARGS.email, ARGS.app_key, ARGS.date_file,
                             ARGS.coords_file, ARGS.products, ARGS.desired_dir,ARGS.addl_info)
