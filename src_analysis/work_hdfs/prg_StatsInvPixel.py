...
# my libraries
hdf_libdir = '/home/tkurihana/clouds/src_analysis/lib_hdfs' # change here
...
def check_invalid_clouds_array(patches, 
                               clouds_mask, 
                               fillvalue_list,
...
def save_file(filename, 
  ...
  np.savez(outputdir+"/"+outputname+'_'+date,
           filename=np.asarray([filename]),
           pixel_list=np.asarray(inv_pixel_list),
           patch_list=np.asarray(patches_list)
  )
  # show to check
  n_inv_pixel = np.sum(inv_pixel_list)
  n_inv_patch = np.sum(
    [patches_list[idx] for idx, i in enumerate(inv_pixel_list) if i > 0]
  )
  print(" ### SAVE FILE %s ==> # inv. pixel: %d | # inv patch: %d ###" 
          % (filename, n_inv_pixel , n_inv_patch ))

def run_main(inputdir = './',
             mod35_datadir ='./',
             outputdir= './',
             outputname='output'
            ):
  ...
  for filename in filelist:
    ...
    if not os.path.exists(m35_file):
      ...
      raise NameError(" ! Program Terminated: Do not exist file !"  )
...
if __name__ == "__main__":
  ...
  # main
  run_main(inputdir=args.mod02_datadir, 
           mod35_datadir=args.mod35_datadir,
