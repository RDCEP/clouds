...
def get_info(mod02_path):
    ...
    mod35 = glob.glob(MAIN_DIR + '/MOD35*.hdf')
    if mod35:
        mod35_path = mod35[0]
...
if __name__ == "__main__":
    ...
    with open(args.outputfile, 'w') as csvfile:
        ...
        outputwriter.writerow(['filename', 'patch_no', 'inval_pixels'])
    csvfile.close()
    desired_files = glob.glob(MAIN_DIR + '/MOD02*.hdf')
    pool.map(get_info, desired_files)
    pool.close()
    pool.join()
