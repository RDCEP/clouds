...
def get_href_lists2(url, keyword):
    ...
    Inputs:
        url: a url
        keyword: string w/ keyword for relevant hrefs
...
def bool_fsize2(url, thresval=1):
    ...
    '''
    coef = 1/1000/1000
    # update: No 'content-length' key in header 
    #r = requests.get(url)
    #fsize = len(r.content) * coef

    #version 2
    # Assume url is filepath downloading in local storage
    bfsize = 0
    try:
        bfsize = os.path.getsize(url)
    except OSError as e:
        print(f"File Not Exist. Please check download directory {e}")
    # Convert to MB and get True/False flag to cut off 
    fsize = bfsize * coef
    print(fsize, bfsize)
    return fsize >= thresval


def download_data(url, start_time, outputdir='.', appkey=None):
    '''
    ...
    # 2020 Tak udpate version: add 
    #set executable line with wget
    try:
...
def combining_fn(iline, url, thresval, outputdir, start_time, keyword, appkey):
    '''
    ...
    Inputs:
        ...
        keyword(str): keyword to find relevant images
    ...
    if response.status_code == 200:
        ...
        for ihref in href_list:
            ...
            if response.status_code == 200:
                ...
                if not bool_fsize2(os.path.join(outputdir, bfilename), thresval):
                  try:
                    os.remove(os.path.join(outputdir, bfilename))
                    print('### Delete {} ###'.format(os.path.join(outputdir, bfilename)))
                  except OSError as e:
                    print(e)
            else:
...
def download_from_name(file, keyword, outputdir, start_time, appkey):
    '''
...
if __name__ == "__main__":
    ...
    p.add_argument('--appkey', type=str, default=None)
    args = p.parse_args()
            ...
            with open(args.datedata, 'r') as ifile:
                for iline in ifile.readlines():
                    args_lst.append((iline, base_url, args.thresval,
                                     args.outputdir, start_time,args.keyword, args.appkey))
        # To generate random dates between two specified dates
            ...
            for iline in datedata:
                args_lst.append((iline, base_url, args.thresval, args.outputdir,
                                 start_time,args.keyword, args.appkey))
        print('here')
        pool.starmap_async(combining_fn, args_lst)    
        #pool.starmap(combining_fn, args_lst)    
        print('here')
