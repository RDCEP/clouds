...
with open("filelist_metadata_random-"+str(nn)+".txt", 'w') as ofile:
...
# train
model = method.train(sc.parallelize(data), k=clusters)
...
patches_list = []
for _data in data:
  patches_list += [model.predict(_data)]
patches_labels = np.asarray(patches_list)
...
np.save(outputdir+'/sd_p-'+str(scaler)+'_nc-'+str(clusters)+'_'+str(mname)+'_spark_patches_labels_'+str(oname), 
        patches_labels)
