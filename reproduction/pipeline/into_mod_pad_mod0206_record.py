...
from alignment_lib import gen_mod06_cwpimg
from alignment_lib import get_allsky_idx
...
def gen_patches(swaths, stride=64, patch_size=128, rpatch_size=128, channels=6,
    ...
    * Generic document by Casper
    ...
    for fname, swath in swaths:
      ...
      m06_file = get_filepath(fname, mod06_datadir, prefix='MOD06_L2.A')
      ...
      if not m06_file is None:
        hdf_m06 = SD(m06_file, SDC.READ)
        if hdf_m06 is None:
          # call mpiabort function
          raise NameError("MOD06 Path does not match any file")
          sys.excepthook =  mpiabort_excepthook
          sys.excepthook = sys.__excepthook__  # 2nd call?
    
        # Pre-process mod06
        """ Process hdf_m06 file and then make mod06(cloud water path) patches[normalized 0-1] 
        """
        allsky_idx = get_allsky_idx(clouds_mask_img)
        mod06_img = gen_mod06_cwpimg(hdf_m06, allsky_idx)
        ...
        for i, j in coords:
          ...
          ### MOD06 data (mean, stdv)
          cwp_mean = np.nanmean(mod06_img[i:i + patch_size, j:j + patch_size])
          cwp_stdv = np.nanstd(mod06_img[i:i + patch_size, j:j + patch_size])
            ...
            if clouds_flag:
              if not patch is None:
                if patch_size != rpatch_size:
                  ...
                  yield fname, (i, j), return_clouds_patch, cwp_mean, cwp_stdv
...
def write_feature(writer, filename, coord, patch, cwp_mean, cwp_stdv):
    feature = {
        ...
        "cwp_mean": _bytes_feature(cwp_mean.tobytes()),
        "cwp_stdv": _bytes_feature(cwp_stdv.tobytes()),
    }
...
def get_args(verbose=False):
    ...
    if verbose:
        for f in FLAGS.__dict__:
            print("\t", f, (25 - len(f)) * " ", FLAGS.__dict__[f], flush=True)
        print("\n", flush=True)
...
if __name__ == "__main__":
    ...
    patches = gen_patches(swaths, 
                          ...
                          mod06_datadir=FLAGS.mod06_datadir, 
