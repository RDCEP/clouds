...
from alignment_lib import gen_mod02_img_mosaic
from alignment_lib import gen_mod35_img_mosaic
# TODO Update
from alignment_lib import get_filepath
...
def gen_sds_mosaic(hdf_datadir,datelist):
  """
      hdf_datadir : MOD02 datadir
      datelist    : list of dates in hdf data directory as 'YYYYDDD' style
  """ 

  for idate in datelist:
     swath = gen_mod02_img_mosaic(hdf_datadir, date=idate)
     if not swath is None:
       ifile = glob.glob(
          os.path.join(hdf_datadir, '*'+str(idate)+'*EV_1KM_Emissive_1.hdf')
       )[0]
       yield ifile, swath
...
def gen_patches(swaths, stride=64, patch_size=128, 
                 ...
                 mod35_datadir='./', thres=0.3, date='2015000'):
    ...
    * Generic document by Casper
    ...
    for fname, swath in swaths:
      ...
      clouds_mask_img = gen_mod35_img_mosaic(mod35_datadir, date=date)
...
if __name__ == "__main__":
    ...
    for i, f in enumerate(sorted(glob.glob(FLAGS.source_glob))):
        if i % size == rank:
            dateinfo = re.findall('[0-9]{7}.mosaic' , f)
            date     = dateinfo[0].rstrip("['").lstrip("']").strip('.mosaic')
            dnames.append(str(date))

    dnames = np.unique(np.array(dnames))
    ...
    swaths  = gen_sds_mosaic(
        os.path.dirname(FLAGS.source_glob),dnames)
    ...
    patches = gen_patches(swaths, FLAGS.stride, FLAGS.shape, 
                          ...
                          thres=FLAGS.thres_cloud_frac,date=dnames[0])
