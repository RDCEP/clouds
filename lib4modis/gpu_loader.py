...
def _decode_cloud_flag(sds_array, fillna=True):
    ...
    def bits_stripping(bit_start,bit_count,value):
        ...
        return cupy.right_shift(cupy.bitwise_and(value,bitmask),bit_start)
    cm_array = sds_array.get()
    cm_array = cupy.asarray(sds_array)
    _, nx, ny = cm_array.shape
    carray = cupy.zeros((nx,ny))
    for ix in range(nx):
        for iy in range(ny):
            cloud_mask_flag = bits_stripping(1,2,cm_array[0,ix,iy])
            carray[ix, iy] = cloud_mask_flag
    ncarray = carray.astype(cupy.float64)
    if fillna:
        nan_idx = cupy.where(cm_array[0] == sds_array.attributes()['_FillValue'])
        ncarray[nan_idx] = cupy.nan
    return ncarray
...
def decode_cloud_flag(sds_array, fillna=True,byte_index=0):
    ...
    cm1_array = cupy.asarray(cm1_array)
    cm1_array = cupy.squeeze(cm1_array[:,:,byte_index])

    ## speed up
    ncarray = cupy.vstack(list(map(lambda x: bits_stripping(1,2,x), cm1_array))).astype(cupy.float64)
    if fillna:
        nan_idx = cupy.where(cm1_array == sds_array.attributes()['_FillValue'])
        ncarray[nan_idx] = cupy.nan
    return ncarray
...
def _decode_ocean_flag(sds_array, fillna=True):
    ...
    def bits_stripping(bit_start,bit_count,value):
        ...
        return cupy.right_shift(cupy.bitwise_and(value,bitmask),bit_start)
    cm_array = sds_array.get()
    cm_array = cupy.asarary(cm_array)
    _, nx, ny = cm_array.shape
    carray = cupy.zeros((nx,ny))
    for ix in range(nx):
        for iy in range(ny):
...
def decode_ocean_flag(sds_array, fillna=True,byte_index=0):
    ...
    def bits_stripping(bit_start,bit_count,value):
        ...
        return cupy.right_shift(cupy.bitwise_and(value,bitmask),bit_start)
    cm_array = sds_array.get()
    cm_array = cupy.asarray(cm_array)
    cm_array = cupy.squeeze(cm_array[:,:,byte_index])
    ncarray = cupy.vstack(list(map(lambda x: bits_stripping(6,7,x), cm_array))).astype(cupy.float64)
    if fillna:
        nan_idx = cupy.where(cm_array[0] == sds_array.attributes()['_FillValue'])
        ncarray[nan_idx] = cupy.nan
    return ncarray
...
def translate_const_clouds_array(patch, clouds_mask, width=128, height=128, thres=0.3, coord=(0,0)):
    ...
    clouds_flag = False
    # main process
    if cupy.any(clouds_mask[i:i+width,j:j+height] == 0):
        tmp = clouds_mask[i:i+width,j:j+height]
        nclouds = len(cupy.argwhere(tmp == 0))
...
def translate_const_ocean_array(patch, ocean_mask, width=128, height=128, thres=0.99, coord=(0,0)):
    ...
    ocean_flag = False
    # main process
    if cupy.any(ocean_mask[i:i+width,j:j+height] == 0):
        tmp = ocean_mask[i:i+width,j:j+height]
        noceans = len(cupy.argwhere(tmp == 0))
...
def proc_sds(sds_array, sdsmax=32767):
    ...
    array = sds_array.get()
    array = cupy.asarray(array).astype(cupy.float64)
    # check bandinfo
    ...
    err_idx = cupy.where( (array > sdsmax) 
                  & (array < sds_array.attributes()['_FillValue']) )
    # nan process
    nan_idx = cupy.where( array == sds_array.attributes()['_FillValue'])
    if len(nan_idx) > 0:
        array[nan_idx] = cupy.nan
    else:
        pass
    if len(err_idx) > 0:
        array[err_idx] = cupy.nan
    else:
        pass
    ...
    offset_array = cupy.zeros(array.shape) # new matrix
    offset_ones  = cupy.ones(array.shape)  # 1 Matrix 
    ...
    scales_array = cupy.zeros(array.shape) # new matrix
...
def aug_array(ref_array, ems_array, ref_bands=[], ems_bands=[], cref_bands=[], cems_bands=[]):
    ...
    # concatenation
    return cupy.concatenate(array_list, axis=2)
...
def make_scale_mask_operator(patches, filename, coordinate, gmean, gstdv, channels=6,nsigma=2):
    ...
    def get_masks(rpatch_size, channels):
        ...
        mask = cupy.expand_dims(mask, axis=-1)
        #  multiple dimension
        mask_list = [ mask for i in range(channels)]
        masks = cupy.concatenate(mask_list, axis=-1)
        return masks
    ...
    upper_index = cupy.where(patches > 1.000)
    patches[upper_index] = 1.000

    # just in case remove negative values
    zero_index = cupy.where(patches < 0.000)
    ...
    if patches.ndim == 4:
        ...
        patches = patches * cupy.asarray(mask)
    elif patches.ndim == 3:
