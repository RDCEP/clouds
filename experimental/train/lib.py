...
def add_noise(imgs, salt_pepper, gaussian_noise):
    ...
    """
    if gaussian_noise:
        imgs += tf.random_normal(tf.shape(imgs), stddev=gaussian_noise)
    ...
    if salt_pepper:
        ...
        noised = tf.cast(tf.random_uniform(tf.shape(img)[:3]) < salt_pepper, tf.float32)
        noised = tf.expand_dims(noised, 3)
        salt = tf.cast(tf.random_uniform(tf.shape(img)[:3]) > 0.5, tf.float32)
...
if __name__ == "__main__":
    ...
    # TODO use initalizable iterator for increasing batch size
    iterator = dataset.make_initializable_iterator()
    _, _, img = iterator.get_next()
    ...
    ckpt_saver = tf.train.Checkpoint(**save_models, **optimizers)

    class SaveKerasModels(tf.train.CheckpointSaverListener):
        def after_save(self, session, global_step_value):
            # for m in save_models:
            #     sname = f"{m}-{global_step_value}.h5"
            #     print("Attempting to save:", sname)
            # save_models[m].save_weights(path.join(FLAGS.model_dir, sname))
            ckpt_saver.save("ckpt-saver")

    hooks = [
        # tf.train.CheckpointSaverHook(
        #     path.join(FLAGS.model_dir, "ckpt"),
        #     save_steps=10,
        #     # save_secs=10,
        #     # saver=ckpt_saver,
        #     saver=tf.train.Saver(),
        #     checkpoint_basename="model.ckpt",
        #     listeners=[SaveKerasModels()],
        #     # BUG "RuntimeError: Graph is finalized and cannot be modified."
        # ),
        tf.train.SummarySaverHook(
            save_secs=20,
            output_dir=path.join(FLAGS.model_dir, "summary"),
            summary_op=tf.summary.merge_all(),
        ),
        tf.train.ProfilerHook(
            save_secs=10, output_dir=path.join(FLAGS.model_dir, "profile")
        ),
        tf.train.NanTensorHook(loss_ae),
        tf.train.StopAtStepHook(100_000),
    ]

    session_creator = tf.train.ChiefSessionCreator(
        scaffold=tf.train.Scaffold(
            init_op=[tf.global_variables_initializer(), iterator.initializer],
            init_feed_dict={batch_size: FLAGS.batch_size},
        ),
        config=tf.ConfigProto(
            gpu_options=tf.GPUOptions(allow_growth=True), log_device_placement=False
        ),
    )
    ...
    with tf.train.MonitoredSession(session_creator, hooks) as sess:
        print("Session started")
        while not sess.should_stop():
            sess.run(train_ops, options=run_opts, run_metadata=run_metadata)
            global_step_value = sess.run(global_step)
            print(global_step_value)
            if global_step_value % 5 == 0:
                ckpt_saver.save("ckpt-saver", sess)
            # if global_step_value % 5 == 0:
            #     for m in save_models:
            #         sname = f"{m}-{global_step_value}.h5"
            #         print("Attempting to save:", sname)
    ...
    # TODO remove the old code once monitored training works
    #
    ...
    #     sess.run(
    #         [tf.global_variables_initializer(), iterator.initializer],
    #         options=run_opts,
    #         run_metadata=run_metadata,
    #         feed_dict={batch_size: FLAGS.batch_size},
    #     )
    #
